...
Name the adapter in `control.skill_name` and configure its trusted entrypoint at `runtime.control_command` in the controller side of `runtime.workspace_image`. Configure `runtime.control_config.worker_command` to start the target app in the separate worker and `runtime.control_config.app_url` to address it at the isolated `benny-worker` host. The controller never mounts the worktree; neither side receives host credentials.

Set the completed user-facing feature map path in `control.feature_map_path`. Copy and fill [`feature-map.example.md`](./feature-map.example.md) outside `.omp/automations/benny/` instead of editing the copied example.
