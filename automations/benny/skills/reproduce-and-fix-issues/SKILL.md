...
- Every child prompt must explicitly forbid every Slack write, including `chat.postMessage`; no Slack write operation exists in a delegated session's tool surface.
...
Prefer the configured Slack actions bound by the runner. Use the optional bot token named by `slack.optional_bot_token_env` only when the user configured it for a narrow missing capability such as editing this one status message. Never expose the token to a worker.
...
Read `references/control-adapter.md` and the completed map at `control.feature_map_path`. The host coordinator drives the app through `runtime.control_command` in a separate trusted controller container; repository reads, commands, and permitted fix writes stay in the untrusted worker container. The controller never mounts the worktree.
