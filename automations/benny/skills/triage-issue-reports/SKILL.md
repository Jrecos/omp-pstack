...
- Every delegated session receives read-only bound actions only. Its prompt must forbid every Slack write, including `chat.postMessage`; no Slack write operation exists in its tool surface.
...
Read the committed routing map at `routing.map_path` (required by the config; it may be an EMPTY map, which means no routes and no owner routing — say the owner is unclear and apply the fallback-free verdict).
...
## 6. Use the bundled Linear tracker

The tracker is the bundled bounded Linear adapter shipped with the runner; no other tracker module executes.
...
7. The Linear tracker can compensate if the verdict post fails.
...
If this run created a tracker issue and the verdict did not land, use the Linear tracker's compensation action. Verify that the issue is canceled, closed, or deleted. If compensation cannot be verified, report the failure only in the automation run output.
