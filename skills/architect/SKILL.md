...
Use the configured `architect runners` from the `pstack-models` profile rule (written by `/setup-pstack`; query it with `pstack_models` action `show`). If the role is unset or any selected model is unavailable in this session, run `/setup-pstack` first — never substitute a model outside the approved pool.
