...
Chat history lives in the session record of the workspace. Reach it with the `pstack_history` tool (`action: "list"` finds the chats, `action: "read"` pulls entries; default window the last seven days of `ctx.cwd`, `all: true` to lift the time limit without lifting workspace isolation). It already excludes the current session and subagent/eval/test sessions, preserves actual modification-time ordering, and resolves citations to real session UUIDs. Never read another workspace's sessions without being asked.
...
3. Fan out across your chat history. List the in-scope chats with `pstack_history` (`action: "list"`, scoped as locked in step 2), then spawn parallel subagents on a fast, cheap model, each taking a slice of the corpus, since searching transcripts is grunt work. Tell every subagent to order candidates by real modification time (`pstack_history` ordering, never by UUID name), grep the topic first and then read only the matching chats and only their relevant regions (`pstack_history` action `read`, or the session JSONL path it names), and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
