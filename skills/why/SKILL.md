...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, the working tree, what was just discussed). State your interpretation briefly so the user can redirect if you're off, then proceed.
...
Before spawning investigators, list the MCP tools available in this session through native MCP discovery: the configured MCP servers, their names, server instructions, tool names, and resource descriptors.
