...
Every serious project needs a scripted way to drive the real app and prove behavior. This skill generates a project-local skill at `<skill-root>/verify-<app>/`. The output is for the next agent, which will read it cold during a task.
...
Read and follow the packaged OMP skill-authoring guidance (`skills/poteto-mode/references/omp-create-skill.md`). Select `<skill-root>` through its **Placement** section before creating files.

Write `<skill-root>/verify-<app>/SKILL.md` with YAML frontmatter (`name: verify-<app>` and a `description` that names the app, the user-facing interface, and when to use the skill) and these sections:

- **Launch:** the exact command that starts the app for verification, and how to tell it's ready (a log line, a port answering, a prompt). Include teardown. For a short-lived CLI or TUI there is no server to keep alive: launch means build the binary (or install deps) once, then start each drive in its own isolated PTY-backed supervised process (spawn it, drive it, stop it; never reuse a live instance across drives).
