...
1. Dispatch a native `task` call with `agent` set to the shipped `Comment Sicko` agent (the plugin's readonly reviewer; tools `read`, `grep`, `glob`, `web_search`, no spawns). Pass the scope: the caller's files or diff, or the scoped diff/source against the base branch. Do not restate its rules. If you need a deeper how/why on a flagged symbol, run that investigation yourself; Comment Sicko cannot spawn.
