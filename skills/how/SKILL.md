...
In every path below, `pstack_agent` has already resolved the configured model into the prepared agent. Pass only its returned `agent` to native `task`; the returned `model` is metadata for reporting the resolved selection.
