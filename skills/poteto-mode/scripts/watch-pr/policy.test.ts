...
describe("mergeability gate", () => {
  const cleanSnapshot = (mergeable: PullRequestFacts["mergeable"]) =>
    readSnapshot({
  ...
  it("never reports READY while mergeability is UNKNOWN, even with clean CI and approvals", async () => {
    const snapshot = await cleanSnapshot("UNKNOWN");
    const waiting = {
      kind: "waiting",
      frontier: context(1),
      pending: null,
    ...
    const stack = [snapshot] satisfies NonEmpty<PrSnapshot>;
    expect(selectTierMajorStackDecision(stack)).toEqual(waiting);
  });

  it("reports READY only once GitHub confirms MERGEABLE", async () => {
    expect(classifyPr(await cleanSnapshot("MERGEABLE"))).toMatchObject({
      kind: "ready",
      pr: { kind: "ready-pr", proof: { mergeability: "clear" } },
    });
  });

  it("keeps CONFLICTING a merge-conflict blocker, not a wait", async () => {
    expect(classifyPr(await cleanSnapshot("CONFLICTING"))).toMatchObject({
      kind: "blocker",
      blocker: { kind: "merge-conflicts" },
    });
  });

  it("names pending checks when they accompany UNKNOWN mergeability", async () => {
    const snapshot = await readSnapshot({
      reader: fakeReader({
  ...
  it("stays nonterminal while UNKNOWN and reaches READY once MERGEABLE", async () => {
    const polling = { ...options, timeout: 30 };
    const emitted: ProgressVerdict[] = [];
    const verdict = await runSimple({
      dependencies: {
        reader: fakeReader({ facts: { mergeable: "UNKNOWN" } }),
        clock: {
          now: () => emitted.length * polling.interval,
          observedAt: () => "2026-07-26T00:00:00.000Z",
          async sleep() {},
        },
    ...
    expect(verdict.reason).toEqual({ kind: "mergeability-unknown" });
    const waiting = emitted.find((event) => event.kind === "WAITING");
    expect(waiting).toMatchObject({
      frontier: context(1),
      reason: { kind: "mergeability-unknown" },
    });

    const ready = await runSimple({
      dependencies: {
        reader: fakeReader(),
        clock: {
          now: () => 0,
    ...
    expect(ready).toMatchObject({
      kind: "READY",
      exitCode: 0,
      scope: { kind: "single", pr: { kind: "ready-pr" } },
    });
  });
});
