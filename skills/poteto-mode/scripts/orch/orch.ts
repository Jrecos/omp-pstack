...
// Guard BEFORE any module load: createRequire.resolve walks this package's
// node_modules with plain Node resolution (no Bun auto-install), so running
// from an arbitrary cwd with a half-installed plugin prints an actionable
// hint instead of a bare resolve error or a silent cache install.
try {
  createRequire(import.meta.url).resolve("commander");
} catch {
  process.stderr.write(
    "pstack orch requires commander. Run `bun install` in the omp-pstack package root and retry.\n",
  );
  process.exit(1);
}
const {
