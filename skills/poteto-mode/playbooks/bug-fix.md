...
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured bug-fix model (set via `/setup-pstack`; if unset or unavailable, run setup first) with a specific scope; review the diff.
