...
- **Coordinator (this chat).** Local. Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are always tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself on repos where local git is cheap; queueing finished work behind an idle stacker is how a deadline harvests nothing. The loop is agentic end to end. Agents are spawned, drained, steered, and revived only through the native `task` tool and the Agent Hub (Alt+A). State reads and writes go through the packaged `orch` helper (the installed-root launcher defined under Store layout) at drain points, one command in and one line out, to conserve context. The CLI is never run from a project-relative `scripts/` path: a marketplace session resolves against the target project, not the plugin.
- **Sub-coordinator.** Always local, durable, one per track, and only when the program exceeds what one coordinator's drains can manage. A track the coordinator can drain itself needs no middle layer: each nested layer re-pays a full orientation preamble, and a blocking sub-coordinator hides its children while the parent idles. Owns its track's units and boards, authors its workers' briefs, spawns its own workers and verifiers (nesting is bounded by the host recursion cap, two nested levels by default). Rolls up aggregates at wave boundaries; never forwards raw child reports. Cap in-flight children at what one drain can process, roughly ten, as a rolling window; never as blocking batches, which cost the slowest child of every batch.
- **Worker / verifier.** Default each worker to its own isolated native task agent (own worktree) unless the task needs this machine: `omp-control-ui` or `omp-control-cli` runtime verification (from the packaged references); reading local session history via the native `pstack_history` tool; simulators and local IDE state; auth that exists only here. Dispatched task agents cannot see your session context, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model family from its worker.
...
Create `orchestrate/<project-slug>/` in the current agent's store (path in the system prompt). The helper lives inside the installed plugin, not in the project: resolve the installed plugin root once with the documented `PSTACK_ROOT` resolver (README, *running the `pstack` CLI after install*; inside the plugin's own checkout, just `PSTACK_ROOT="$(pwd)"`), then run bookkeeping as the stable subcommand `pstack orch` — `bun "$PSTACK_ROOT/src/cli.ts" orch` — written below as `orch`, while its canonical plain TSV and JSON stay readable without the CLI.
...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape; a 4KB scaffold around a two-line edit costs more to write and obey than the edit. Spawns may reference the standing-orders file by store path; every resume pastes it verbatim.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget with the isolated-worktree default and the shared-machine exception list, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line; plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher wake (arm it via the native `/loop`, with a long heartbeat fallback), and before a human report. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Restacks run in their own task-agent worktree, never the coordinator's interactive checkout; a restack at this scale takes the machine down.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the agent's status in the Agent Hub (Alt+A). Transcript mtime is not liveness.
...
- After an OMP restart: task agents are gone; their pushed branches and PRs are not. Re-read the standing orders and `units.tsv`, recompute the frontier, reattach surviving work by PR and branch rather than agent id, respawn one sub-coordinator per track from its stored brief plus current state, drain, resume. The dead session's store lock clears itself on the next write; `orch` replaces a lock whose holder pid is gone.
