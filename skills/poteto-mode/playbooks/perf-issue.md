...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured perf-issue model (set via `/setup-pstack`; if unset or unavailable, run setup first); review the diff. Capture a post-fix trace.
