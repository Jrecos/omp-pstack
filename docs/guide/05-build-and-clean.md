...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs `/deslop` on the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. The deslop checks ship with this plugin as [`omp-deslop`](../../skills/poteto-mode/references/omp-deslop.md). If you don't have it loaded, ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
The division of labor is worth keeping straight. The packaged [`omp-deslop` checklist](../../skills/poteto-mode/references/omp-deslop.md) cleans slop out of the code, `/unslop` cleans it out of prose, and `/no-comments` hands the comments to a reviewer who didn't write them.
