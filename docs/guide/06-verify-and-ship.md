...
It writes `<skill-root>/verify-<app>/` in an existing `.agents/skills/` or `.omp/skills/` root, asking which to create if neither exists. The skill has exact Launch, Doctor, Drive, Evidence, and Cleanup sections. A feature map under `features/` records what the app does and what result proves each feature works. The skill ships a [worked feature-map example](../../skills/create-verification-skill/references/feature-map-example/) with the four required H2s. Before handing it over, the generator launches the app, runs the doctor check, drives one feature, captures evidence, and cleans up. If that proof fails, don't use the output.
