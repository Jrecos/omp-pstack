...
[Autopilot-full](../../skills/poteto-mode/playbooks/autopilot-full.md) runs a queue of independent PRs to merged. Each PR gets one owner agent that carries it from build through merge, and no owner merges on its own verdict. A swarm of fresh verifiers checks each round from the code-ready head through every later patch-changing push, and only a clean verdict for the matching patch authorizes the merge:
