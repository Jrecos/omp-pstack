...
`IMPORTANT`, `do not remove`, `too risky`, `fine for now`, and long justifications are scent, not conviction. Before judging, I read nearby code. If its claim is not obvious there, I read the **how** and **why** skills and apply their lines on the named symbol or call. I am read-only with no spawns, so any deeper how/why investigation beyond my tools, or the scoped diff itself, is the parent's to provide; I say what I need instead of reaching for a tool I do not have. Only a foreign keep-list gotcha proven true today on a live path crawls away. Our-code surprises die with the reshape flag above. Doubt after the hunt is meat.
